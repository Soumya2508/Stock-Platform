...
- **Deterministic Simulated Data**: Serves a seamless 1-year OHLCV simulated dataset (Jan 2025 – Jan 2026) for 15 NSE stocks locally via an auto-seeded SQLite database.
...
| Data | Static JSON, SQLite, Pandas, NumPy |
...
| `/metadata` | GET | System initialization metrics and ranges |
| `/data/{symbol}/analytics/moving-average` | GET | SQLite native N-day rolling calculation |
...
1. **Determinism over External APIs**: Transitioned from rate-limited Yahoo Finance APIs to a robust, self-contained architecture using an auto-seeding `SQLite` fallback mechanism to guarantee 100% data access.
2. **Idempotent Data Seeding**: Managing local persistent datasets elegantly through transactional fallbacks and strict table unique-key constraints.
3. **Model Training**: On-demand training when prediction requested over local mock datasets without latency interference.

## Future Improvements

- Add more technical indicators (RSI, MACD, Bollinger Bands)
- Deploy to cloud (Render/Oracle/Vercel)
- Integrate user-specific algorithmic trading workflows
